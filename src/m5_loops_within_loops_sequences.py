...
def run_test_integers():
    ...
    # Test 2:
    expected = [5, 4, 3, 1, 0, 1, 2]
    answer = integers([('hi', '5', 5, 4, 3, str(2), 1, 0), (1, 2, 'bye')])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
    ...
    for k in range(len(sequence_of_sequences)):
        for j in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k][j]) == int:
                new_list = new_list + [sequence_of_sequences[k][j]]
    return new_list
    # ------------------------------------------------------------------
...
def run_test_big_letters():
    ...
    answer = big_letters([(1,2), 'And', (3, str(4)), 'Bat', 'Cat', 'DEn', (4, 5), 'Fire', 'God',
                          'HI', 'JacK', 'LaMeNt', 6, 'O$ Pp q r Q rR', 'STUVWXYz', '%Z'])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
        ...
        if type(sequence_of_sequences[k]) == str:
            for j in range(len(sequence_of_sequences[k])):
                if sequence_of_sequences[k][j].isupper():
                    new_string = new_string + str(sequence_of_sequences[k][j])
    return new_string
    # ------------------------------------------------------------------
